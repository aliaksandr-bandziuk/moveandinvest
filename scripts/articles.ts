...
const ENTRIES: Record<string, EntryConfig> = {
  ...
  "malta-citizenship": {
    ...
    sources: {
      ...
      // РУССКАЯ ДОБАВЛЕНА 9 СЕНТЯБРЯ 2026 (пункт плана F6). Не перевод:
      // английская разбирает цепочку актов и живую страницу агентства, русская
      // отвечает на «можно ли ещё купить» и показывает три заголовка 2026 года
      // из собственной выдачи. Про их тела не утверждается ничего — два домена
      // не отдают содержимое, и в тексте это сказано прямо.
      ru: "article-ru-malta-citizenship.md",
    },
    figures: {
      en: ["mt-chain-en", "mt-routes-en"],
      // Те же две схемы; подписи короче английских, потому что кириллица шире.
      ru: ["mt-chain-ru", "mt-routes-ru"],
    },
  ...
  "greece-citizenship": {
    ...
    sources: {
      ...
      // РУССКАЯ ДОБАВЛЕНА 9 СЕНТЯБРЯ 2026 (пункт плана F5) и это НЕ перевод.
      // Английская отвечает на «что говорит закон» и целится в
      // `greek citizenship law`, потому что головной английский запрос — про
      // происхождение. По-русски разбивка другая: происхождение, репатриация и
      // брак дают 360 из 2 000 схлопнутых, а три четверти кластера — «как
      // получить», сроки и порядок. Поэтому русская берёт голову и идёт от
      // «сколько лет нужно именно вам», а структура статей и экзамен уходят
      // ниже.
      ru: "article-ru-greece-citizenship.md",
    },
    figures: {
      en: ["gr-nat-tiers-en", "gr-nat-split-en"],
      // Те же две схемы, но подписи не переведены слово в слово: кириллица шире
      // латиницы при том же числе знаков, и потолки строк здесь свои.
      ru: ["gr-nat-tiers-ru", "gr-nat-split-ru"],
    },
  ...
  "poland-waiting": {
    ...
    // "rules": предмет — статья 100d и то, что она выключает в кодексе, до
    // какой даты и для кого.
    category: "rules",
    // ПУСТО НАМЕРЕННО. Польша — раздел, а не юрисдикция реестра, и `countries`
    // не может её назвать. Что статья о Польше, говорит источник `pl-legal` в
    // её заголовке — и именно он ставит в конец польскую форму вместо блока
    // гайда. См. «The residence form (Poland)» в CLAUDE.md.
    countries: [],
    // РУССКАЯ ТОЛЬКО. Спрос — русские и украинские запросы из Польши
    // («проверка статуса карты побыту» 4 400); украинской локали на сайте нет,
    // польский читатель этого сегмента ищет по-польски шаблоны, а не разбор.
  ...
  "poland-karta-pobytu": {
    ...
    // "rules": предмет — основания временного пребывания, ст. 98–188 закона
    // об иностранцах и ст. 3 закона о работе иностранцев.
    category: "rules",
    // Пусто намеренно: Польша задаётся источником pl-legal.
    countries: [],
    // РУССКАЯ ТОЛЬКО, и украинской локали не будет (решение владельца, 15
    // сентября 2026), хотя «карта побиту» — 14 800 против 2 400 у «карта
    // побыту». Хаб: ссылается на все четыре польские статьи.
  },
  "poland-refusal": {
    ...
    // "rules": предмет — жалоба по KPA, ст. 299–321 закона об иностранцах, суд.
    category: "rules",
    // Пусто намеренно: Польша задаётся источником pl-legal.
    countries: [],
    // РУССКАЯ ТОЛЬКО. Спрос маленький по «отказ» (20), но «негативная децизия»
  ...
  "poland-deportation": {
    ...
    // "rules": предмет — ст. 101–123, 299–336, 465 закона об иностранцах по ситуациям.
    category: "rules",
    // Пусто намеренно: Польша задаётся источником pl-legal.
    countries: [],
    // РУССКАЯ ТОЛЬКО. Первая из страниц «по ситуациям» (просьба владельца, 15
    // сентября 2026). Сценарные запросы по Google Ads — 0–50 в месяц, поэтому
  ...
  "poland-property": {
    ...
    // "rules": предмет — закон 1920 года о приобретении недвижимости иностранцами.
    category: "rules",
    // Пусто намеренно: Польша задаётся источником pl-legal.
    countries: [],
    // РУССКАЯ ТОЛЬКО. Первая страница расширенного раздела (решение владельца
    // 16 сентября 2026): не легализация, а покупка жилья. Русский
    // информационный спрос мал (10–390), польский — ~1 000; выбрана потому,
    // что это услуга партнёрши и прямой ответ на её же возражение клиентов
    // «разрешение на землю нереально».
  },
  "poland-business-pillar": {
    ...
    // которая обновляется, а не заменяется: ведёт на узкие статьи блога, они
    // ссылаются обратно. Решение владельца 17 сентября 2026 года. Польша —
    // раздел, не юрисдикция, и пиллар в корне даёт разделу вход, не трогая
    // реестр стран. «Жизнь в польше» — 10–30 запросов, поэтому общий вход
  ...
  "poland-business": {
    ...
    category: "rules",
    countries: [],
    // РУССКАЯ ТОЛЬКО. Первая узкая статья под пилларом «Бизнес в Польше».
    // Спрос ≈1500 ru + ≈1000 uk, CPC $3–30 — самый денежный кластер раздела.
    // Фирменный счёт — раздел здесь, а не страница: ≈150 в месяц.
    // Содержательные поводы: 5 марта 2026 года отменена ст. 23 для украинцев
    // (Dz.U. 2026 poz. 203), порог НДС 240 000 zł с 1 января 2026 года
    // (Dz.U. 2025 poz. 896), квартальный порог 225% с того же дня.
  },
  "poland-driving": {
    ...
    // "rules": предмет — закон о лицах, управляющих транспортными средствами.
    category: "rules",
    // Пусто намеренно: Польша задаётся источником pl-legal.
    countries: [],
    // РУССКАЯ ТОЛЬКО. Самый большой русскоязычный спрос во всём польском
    // разделе: кластер «замена прав» ≈1450 в месяц против 390 у налогов.
    // По-польски тот же запрос — 0: вопрос задают приезжие, не поляки.
    // Содержательный повод написать: 17 декабря 2025 года поправка Dz.U. 2025
    // poz. 1676 переписала условие экзамена со страны на документ и впервые
    // разрешила менять просроченные иностранные права — ни то ни другое в
    // выдаче не отражено.
  },
  "poland-pesel": {
    ...
    // "rules": предмет — закон об учёте населения, ст. 7, 15–19 и глава 5a.
    category: "rules",
    countries: [],
    // РУССКАЯ ТОЛЬКО. Кириллицей «песель укр» 1600, «песель» 590, «укр
    // песель» 480, «песель в польше» 140 — почти весь спрос украинский и
    // смешанный. Поводы: два пути получения с номерами статей, личная явка по
    // ст. 18 ч. 1a, PESEL UKR по главе 5a после закона Dz.U. 2026 poz. 203.
  },
  "poland-meldunek": {
    ...
    category: "rules",
    countries: [],
    // РУССКАЯ ТОЛЬКО. «мельдунек» 480, «мельдунек это» 210, города по 70–170.
    // Поводы: срок четвёртого дня для иностранцев вне ЕС (ст. 41 ч. 3) против
    // «30 дней» у всех, включая gov.pl; личная явка без PESEL с 1 января 2026
    // года; мельдунек не обязателен для карты побыту (ст. 244 ч. 4 закона об
    // иностранцах); штраф по ст. 59.
  },
  "poland-taxes": {
    ...
    category: "rules",
    countries: [],
    // РУССКАЯ ТОЛЬКО. Кириллицей ≈1300 ru («налоги в польше» 320, «подоходный
    // налог в польше» 170) и ≈500 uk. Поводы: льгота на возвращение по ст. 21
    // ч. 1 п. 152 для владельцев Карты поляка; общий лимит 85 528 zł; возврат
    // неиспользованной льготы на детей; закрытая совместная декларация для
    // супруга вне ЕС; сроки возврата по ст. 77 Налогового кодекса.
  },
  "poland-mortgage": {
    ...
    category: "rules",
    countries: [],
    // РУССКАЯ ТОЛЬКО. Кластер ипотеки ≈1300 ru («ипотека в польше» 320,
    // калькуляторы 210 + 170) и ≈400 uk. Поводы: условия о карте побыту —
    // политика банков, не закон; валюта по ст. 6; решение на 21-й день;
