...
export const CHANGES_PAGE_COPY: Record<Locale, ChangesPageCopy> = {
  en: {
    ...
    intro:
      "Twenty-nine changes to residency and tax rules — in the five jurisdictions this site compares, and in Poland, where its second section is — each with the day it took effect and the instrument that made it. Poland is the fastest-moving of them: three of its five rows took effect in 2026, and one of them closed every channel for filing a residence application except the ministry's own module. Three of them have no instrument at all: Dubai altered two rules in 2026 that thousands of people are now applying under, and Greece has suspended new applications from Russian and Belarusian citizens since 2022 — none of the three has ever been published as a law, a decision or a gazette reference, and we walked the registers to establish it rather than assuming it.",
    howToRead:
  ...
  ru: {
    ...
    intro:
      "Двадцать девять изменений в правилах проживания и налогообложения — в пяти юрисдикциях, которые сравнивает этот сайт, и в Польше, где у него второй раздел, — с днём вступления в силу и с актом, который это сделал. У трёх акта нет вовсе: Дубай изменил в 2026 году два правила, по которым сейчас подают тысячи людей, а Греция с 2022 года не принимает новые заявления от граждан России и Беларуси — и ни одно из трёх никогда не публиковалось ни законом, ни решением, ни номером газеты. Мы прошли реестры и установили это, а не предположили. Польша меняется быстрее всех: три из пяти её строк вступили в силу в 2026 году, и одна из них закрыла все каналы подачи заявления о пребывании, кроме министерского модуля.",
    howToRead:
  ...
  pl: {
    ...
    intro:
      "Dwadzieścia dziewięć zmian w przepisach pobytowych i podatkowych — w pięciu jurysdykcjach, które porównuje ta strona, oraz w Polsce, gdzie ma drugą sekcję — każda z dniem wejścia w życie i aktem, który jej dokonał. Polska zmienia się najszybciej: trzy z jej pięciu wierszy weszły w życie w 2026 roku, a jeden z nich zamknął wszystkie drogi składania wniosku pobytowego poza modułem ministerstwa. Trzy nie mają aktu w ogóle: Dubaj zmienił w 2026 roku dwie zasady, na podstawie których wnioski składają teraz tysiące osób, a Grecja od 2022 roku nie przyjmuje nowych wniosków od obywateli Rosji i Białorusi — i żadna z tych trzech nigdy nie została opublikowana jako ustawa, decyzja ani sygnatura dziennika. Przeszliśmy rejestry i to ustaliliśmy, a nie założyliśmy.",
    howToRead:
