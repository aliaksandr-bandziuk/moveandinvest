...
export interface HomeCopy {
  ...
  /** The unnumbered Poland band. Russian only: the Poland section has no
   *  English or Polish pages, so the other two documents leave it out and
   *  `npm run content` unsets it there. */
  polandBand?: {
    eyebrow: string;
    heading: string;
    intro: string;
    links: { label: string; description: string; slug: string }[];
  };
...
export const HOME_COPY: Record<Locale, HomeCopy> = {
  ...
  ru: {
    ...
    polandBand: {
      eyebrow: "Уже живёте в Польше",
      heading: "Легализация, бизнес и жизнь в Польше",
      intro:
        "Для тех, кто уже живёт в Польше, на сайте есть отдельный раздел — не шестая страна в таблице выше, а разборы по статьям польских законов. Карта побыту и CUKR, постоянное пребывание и гражданство, отказ и сроки, своя фирма и бизнес-инкубаторы, покупка недвижимости и замена прав. Каждая цифра сверена с первоисточником и стоит на странице источников с датой проверки.",
      links: [
        {
          label: "Легализация в Польше",
          description: "Карта побыту, CUKR, ПМЖ и гражданство: условия, сборы и сроки 2026 года.",
          slug: "legalizaciya-v-polshe",
        },
        {
          label: "Бизнес в Польше",
          description: "JDG или sp. z o.o., налоги и ZUS, фирменный счёт и карта побыту на бизнес.",
