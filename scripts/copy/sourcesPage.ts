...
export const SOURCES_PAGE_COPY: Record<Locale, SourcesPageCopy> = {
  en: {
    ...
    intro:
      "Each headline figure here has been read back from the statute, the ministry tariff or the official fee schedule it is supposed to rest on. Sixty-one checks. Nineteen came back wrong, seven could not be established from any primary source at all, and those seven are not published as figures anywhere on this site. The first sitting was 23 August 2026; every row re-read since carries its own date. Below is all of it, including the parts that do not flatter us.",
    howToRead:
    ...
    seo: {
      ...
      metaDescription:
        "The full working behind the comparison: 61 checks against statutes and ministry tariffs, 19 of which corrected a figure. Citations, links and what could not be verified.",
    },
  },
  ru: {
    eyebrow: "Выкладки",
    ...
    intro:
      "Каждая заголовочная цифра здесь перечитана из закона, тарифа ведомства или официальной таблицы сборов, на которых она должна стоять. Шестьдесят одна проверка. Девятнадцать вернулись неверными, семь не удалось установить ни по одному первоисточнику — и эти семь нигде на сайте цифрами не публикуются. Первый заход был 23 августа 2026 года; у каждой перечитанной с тех пор строки стоит своя дата. Ниже всё, включая то, что нас не красит.",
    howToRead:
    ...
    seo: {
      ...
      metaDescription:
        "Полные выкладки за сравнением: 61 сверка с законами и тарифами ведомств, девятнадцать из которых исправили цифру. Ссылки на законы и то, что подтвердить не удалось.",
    },
  },
  pl: {
    eyebrow: "Wyliczenia",
    ...
    intro:
      "Każda główna liczba została tu odczytana na nowo z ustawy, taryfy ministerialnej albo oficjalnej tabeli opłat, na których ma się opierać. Sześćdziesiąt jeden sprawdzeń. Dziewiętnaście wróciło błędnych, siedmiu nie udało się ustalić z żadnego źródła pierwotnego — i tych siedmiu nigdzie na stronie nie publikujemy jako liczb. Pierwsze posiedzenie było 23 sierpnia 2026; każdy czytany od tego czasu wiersz nosi własną datę. Poniżej wszystko, łącznie z tym, co nas nie zdobi.",
    howToRead:
    ...
    seo: {
      ...
      metaDescription:
        "Pełne wyliczenia stojące za porównaniem: 61 sprawdzeń wobec ustaw i taryf ministerialnych, z czego dziewiętnaście poprawiło liczbę. Odesłania do przepisów i to, czego nie udało się potwierdzić.",
    },
  },
};
