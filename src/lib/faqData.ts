  ...
  // /sources need somewhere to be explained rather than buried.
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: {
          en: "Six, and naming them is the point of this answer. Malta's processing time — no primary source publishes one, so the widely quoted four to six months appears nowhere on this site. Whether off-plan property qualifies in the UAE — rechecked across four authorities on 25 August 2026, still nothing in either direction. Two Cypriot figures: the threshold in its regulation 6(2) and the reduced VAT rate with its limits. And where a Portugal D7 application is actually filed, consulate or VFS Global, which is stated only on ministry hosts that did not answer. And one Polish one: an average waiting time for a residence permit, where the only official figures are years old. Each of those is a question we get asked and could answer plausibly in one sentence. A plausible answer is what everyone else publishes, and it is indistinguishable from a true one right up until it costs somebody money.",
          ru: "Шесть — и назвать их и есть смысл этого ответа. Срок рассмотрения на Мальте: его не публикует ни один первоисточник, поэтому ходовые «четыре-шесть месяцев» на этом сайте не встречаются нигде. Годится ли в ОАЭ объект на стадии строительства: перепроверено по четырём ведомствам 25 августа 2026 года — по-прежнему ничего ни за, ни против. Две кипрские: порог в регламенте 6(2) и пониженная ставка НДС с её пределами. И куда подаётся заявление на португальскую D7, в консульство или через VFS Global: это сказано только на узлах министерства, которые не отвечали. И одна польская: средний срок рассмотрения разрешения на пребывание, по которому официальные цифры есть только многолетней давности. Каждый из этих вопросов нам задают, и на каждый можно правдоподобно ответить одной фразой. Правдоподобный ответ — это то, что публикуют все остальные, и он неотличим от верного ровно до того момента, когда обойдётся кому-то в деньги.",
          pl: "Sześciu — a ich nazwanie jest sensem tej odpowiedzi. Czas rozpatrywania na Malcie: nie publikuje go żadne źródło pierwotne, więc obiegowe „cztery do sześciu miesięcy” nie pojawia się na tej stronie nigdzie. Czy w ZEA kwalifikuje się nieruchomość w budowie: sprawdzone ponownie w czterech urzędach 25 sierpnia 2026 — nadal nic ani za, ani przeciw. Dwie cypryjskie: próg z rozporządzenia 6(2) oraz obniżona stawka VAT wraz z jej limitami. I gdzie faktycznie składa się wniosek o portugalską D7, w konsulacie czy przez VFS Global: podają to wyłącznie serwery ministerstwa, które nie odpowiadały. I jedna polska: średni czas rozpatrywania wniosku o zezwolenie pobytowe, dla którego oficjalne liczby są sprzed lat. O każde z tych pytań nas pytają i na każde da się prawdopodobnie odpowiedzieć jednym zdaniem. Prawdopodobna odpowiedź to właśnie to, co publikują wszyscy inni, i jest nie do odróżnienia od prawdziwej dokładnie do chwili, gdy kogoś kosztuje pieniądze.",
        },
      },
    ],
  },
...
const STATED_COUNTS = { checks: 99, corrected: 19, unverified: 6 };
